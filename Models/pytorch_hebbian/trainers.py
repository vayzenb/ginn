...
class Trainer(ABC):
    ...
    def __init__(self, engine, model: torch.nn.Module, device: Optional[Union[str, torch.device]] = None):
        #set_start_method('spawn', force=True) #vlad added
        self.engine = engine
        self.model = model
...
class HebbianTrainer(Trainer):
    ...
    def create_hebbian_trainer(self, model: torch.nn.Module, learning_rule, optimizer, device=None, non_blocking=True,
