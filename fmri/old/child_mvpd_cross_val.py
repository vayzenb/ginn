...
def calc_mvpd(seed_comps,target_comps, target_pca):
    ...
    """
    seed_set1, seed_set2, target_set1, target_set2 = split_data(seed_comps, target_comps)

    all_scores = []
    for pcn in range(0,len(target_pca.explained_variance_ratio_)):
        
        #fit on first half and predict second half
        clf.fit(seed_set1, target_set1[:,pcn]) #fit seed PCs to target
        pred_ts = clf.predict(seed_set2) 

        corr1 = np.corrcoef(pred_ts,target_set2[:,pcn])[0,1]
        
        #fit on second half and predict first half
        clf.fit(seed_set2, target_set2[:,pcn]) #fit seed PCs to target
        pred_ts = clf.predict(seed_set1) 

        corr2 = np.corrcoef(pred_ts,target_set1[:,pcn])[0,1]

        #average the two r2 vals
        corr_mean = (corr1 + corr2)/2

        #weight the rsquared vals by the amount of variance explained by that PC
        weighted_corr = corr_mean * target_pca.explained_variance_ratio_[pcn]
