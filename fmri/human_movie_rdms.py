...
for sub in enumerate(sub_list['sub']):
    ...
    for roi in rois:
        for lr in ['l','r']:
            ...
            rdm, rdm_vec = create_rdm(sub_pcs)
            rdm_df = pd.DataFrame(rdm_vec, columns = ['rdm'])
            np.save(f'{out_dir}/sub-{sub[1]}/derivatives/rdms/{lr}{roi}_rdm.npy',rdm)
            rdm_df.to_csv(f'{out_dir}/sub-{sub[1]}/derivatives/rdms/{lr}{roi}_rdm_vec.csv',index = False)
