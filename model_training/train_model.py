...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        loss_raw, loss_avg = train(train_loader, model, lemniscate, criterion, optimizer, epoch)
        
        writer.add_scalar("Raw Train Loss", loss_raw, epoch) #write to tensorboard
        writer.add_scalar("Avg Train Loss", loss_avg, epoch) #write to tensorboard
        writer.flush()

        # evaluate on validation set
        prec1 = NN(epoch, model, lemniscate, train_loader, val_loader)

        #write Nearest-neighbor acc
        writer.add_scalar("Validation accuracy", prec1, epoch) #write to tensorboard
        writer.flush()
...
def train(train_loader, model, lemniscate, criterion, optimizer, epoch):
    ...
    for i, (input, _, index) in enumerate(train_loader):
        ...
        #async doesnt exist anymore; use non-blocking = False
